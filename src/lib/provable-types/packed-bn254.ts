...
class Packed<T> {
   ...
   */
  static create<T>(type: ProvableExtendedBn254<T>): typeof Packed<T> & {
  ...
  constructor(packed: FieldBn254[], value: Unconstrained<T>) {
    this.packed = packed;
    this.value = value;
  }
  ...
  static _innerProvable: ProvableExtendedBn254<any> | undefined;

  get Constructor(): typeof Packed {
    return this.constructor as typeof Packed;
  }

  static get innerProvable(): ProvableExtendedBn254<any> {
...
function countFields(input: HashInput) {
  ...
  for (let [, bits] of input.packed ?? []) {
    pendingBits += bits;
    if (pendingBits >= FieldBn254.sizeInBits) {
      n++;
      pendingBits = bits;
    }
  }
...
class Hashed<T> {
  ...
  static create<T>(
    ...
    return class Hashed_ extends Hashed<T> {
      ...
      static _hash = _hash satisfies (t: T) => FieldBn254;
  ...
  constructor(hash: FieldBn254, value: Unconstrained<T>) {
    this.hash = hash;
    this.value = value;
  }

  static _hash(_: any): FieldBn254 {
