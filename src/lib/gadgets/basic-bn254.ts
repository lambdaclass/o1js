...
import type { FieldBn254, VarField } from '../field-bn254.ts';
import { existsOne, toVar } from './common-bn254.ts';
import { GatesBn254 } from '../gates-bn254.ts';
 ...
 */
function arrayGet(array: FieldBn254[], index: FieldBn254) {
  let i = toVar(index);
 ...
 */
function assertOneOf(x: FieldBn254, allowed: [bigint, bigint, ...bigint[]]) {
  let xv = toVar(x);
  let [c1, c2, ...c] = allowed;
  let n = c.length;
  if (n === 0) {
