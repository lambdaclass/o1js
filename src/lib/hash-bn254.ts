...
type ProvableHashable<T> = ProvableBn254<T> & Hashable<T>;
...
const Poseidon = {
  hash(input: FieldBn254[]) {
    if (isConstant(input)) {
      return FieldBn254(PoseidonBigint.hash(toBigints(input)));
    }
    return Poseidon.update(Poseidon.initialState(), input)[0];
  },

  update(state: [FieldBn254, FieldBn254, FieldBn254], input: FieldBn254[]) {
    ...
    return MlFieldArray.from(newState) as [FieldBn254, FieldBn254, FieldBn254];
  },

  hashWithPrefix(prefix: string, input: FieldBn254[]) {
  ...
  initialState(): [FieldBn254, FieldBn254, FieldBn254] {
    return [FieldBn254(0), FieldBn254(0), FieldBn254(0)];
  },

  hashToGroup(input: FieldBn254[]) {
    if (isConstant(input)) {
      ...
      return {
        x: FieldBn254(x),
        y: { x0: FieldBn254(y.x0), x1: FieldBn254(y.x1) },
      };
    }
    ...
    let x0 = ProvableBn254.witness(FieldBn254, () => {
...
function hashConstant(input: FieldBn254[]) {
  return FieldBn254(PoseidonBigint.hash(toBigints(input)));
}

const HashHelpers = createHashHelpers(FieldBn254, Poseidon);
...
function prefixToField(prefix: string) {
  ...
  let bits = [...prefix]
    ...
    .flat();
  return FieldBn254.fromBits(bits);
}
...
function packToFields({ fields = [], packed = [] }: HashInput) {
  ...
  let packedBits = [];
  let currentPackedField = FieldBn254(0);
  let currentSize = 0;
  for (let [field, size] of packed) {
    currentSize += size;
    if (currentSize < 255) {
      currentPackedField = currentPackedField
        .mul(FieldBn254(1n << BigInt(size)))
        .add(field);
    } else {
...
const TokenSymbolPure: ProvableExtendedBn254<
  { symbol: string; field: FieldBn254 },
  string
> = {
  toFields({ field }) {
    return [field];
  ...
  empty() {
    return { symbol: '', field: FieldBn254(0n) };
  },
};
...
function isConstant(fields: FieldBn254[]) {
  return fields.every((x) => x.isConstant());
}
function toBigints(fields: FieldBn254[]) {
