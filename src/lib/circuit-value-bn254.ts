...
type Struct<T> = ProvableExtendedBn254<NonMethods<T>> &
  Constructor<T> & { _isStruct: true };
type StructPure<T> = ProvablePureBn254<NonMethods<T>> &
  ProvableExtension<NonMethods<T>> &
  Constructor<T> & { _isStruct: true };
type FlexibleProvableBn254<T> = ProvableBn254<T> | Struct<T>;
type FlexibleProvablePureBn254<T> = ProvablePureBn254<T> | StructPure<T>;
...
abstract class CircuitValue {
  ...
  static toFields<T extends AnyConstructor>(
    ...
    for (let i = 0, n = fields.length; i < n; ++i) {
      ...
      const subElts: FieldBn254[] = propType.toFields((v as any)[key]);
  ...
  static toInput<T extends AnyConstructor>(
    ...
    for (let i = 0, n = fields.length; i < n; ++i) {
      ...
      let xs: FieldBn254[] = type.toFields(v[key]);
  ...
  equals(x: this) {
    return ProvableBn254.equal(this, x);
  }

  assertEquals(x: this) {
    ProvableBn254.assertEqual(this, x);
  }
  ...
  static toConstant<T extends AnyConstructor>(
    ...
    const xs: FieldBn254[] = (this as any).toFields(t);
...
function arrayProp<T>(elementType: FlexibleProvableBn254<T>, length: number) {
  return function (target: any, key: string) {
    if (!target.hasOwnProperty('_fields')) {
      target._fields = [];
    }
    target._fields.push([key, ProvableBn254.Array(elementType, length)]);
  };
}

function matrixProp<T>(
  elementType: FlexibleProvableBn254<T>,
  nRows: number,
  nColumns: number
) {
  return function (target: any, key: string) {
    ...
    target._fields.push([
      key,
      ProvableBn254.Array(ProvableBn254.Array(elementType, nColumns), nRows),
    ]);
  };
}
 ...
 * ```ts
 * let Vote = { hasVoted: BoolBn254, inFavor: BoolBn254 };
 *
 ...
 *   votes: [Vote, Vote, Vote]
 * }) {
 *   vote(index: number, inFavor: BoolBn254) {
 *     let vote = this.votes[i];
 *     vote.hasVoted = BoolBn254(true);
 ...
 * ```ts
 * let emptyVote = { hasVoted: BoolBn254(false), inFavor: BoolBn254(false) };
 * let voter = new Voter({ publicKey, votes: Array(3).fill(emptyVote) });
 * voter.vote(1, BoolBn254(true));
 * ```
 *
 * In addition to creating types composed of FieldBn254 elements, you can also include auxiliary data which does not become part of the proof.
...
>(
  ...
  : ProvableBn254<T>) & {
    toInput: (x: T) => {
      fields?: FieldBn254[] | undefined;
      packed?: [FieldBn254, number][] | undefined;
    };
    toJSON: (x: T) => J;
    fromJSON: (x: J) => T;
    empty: () => T;
  ...
  class Struct_ {
     ...
     */
    static toFields(value: T): FieldBn254[] {
     ...
     */
    static fromFields(fields: FieldBn254[], aux: any[]) {
...
>(
  ...
  return Struct(type) satisfies ProvableBn254<T> as any;
}
 ...
 *
 *     ProvableBn254.witness(FieldBn254, () => {
...
and ProvableBn254.asProver() blocks, which execute outside the proof.
`);
   ...
   *
   * ```ts
   * let xWrapped = Unconstrained.witness(() => ProvableBn254.toConstant(type, x));
   * ```
   */
...
let primitives = new Set([FieldBn254, BoolBn254]);
...
function circuitValueEquals<T>(a: T, b: T): boolean {
  ...
  if ('equals' in a && typeof (a as any).equals === 'function') {
    ...
    if (isEqual instanceof BoolBn254) return isEqual.toBoolean();
  }
  ...
  if (
    ...
    let aFields = (a as any).toFields() as FieldBn254[];
    let bFields = (b as any).toFields() as FieldBn254[];
...
function toConstant<T>(type: FlexibleProvableBn254<T>, value: T): T;
function toConstant<T>(type: ProvableBn254<T>, value: T): T {
